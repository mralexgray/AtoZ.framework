...
#define BASICTABLEVIEWDRAGDROP(mutableObjectArrayKey, tableViewKey) \
static NSS* dragTypeDeclaration = nil;\
+ (void) load { \
	[$ swizzleMethod:@selector(awakeFromNib) with:@selector(swizzleawakeFromNib) in:self.class];\
	dragTypeDeclaration = @"com.mrgray.BASICTABLEVIEWDRAGDROP"; } \
- (void) swizzleawakeFromNib {  \
	[self swizzleawakeFromNib]; \
	NSTV*t = [self valueForKey:tableViewKey]; \
	[t setDataSource:(id<NSTableViewDataSource>)self]; [t registerForDraggedTypes:@[dragTypeDeclaration]]; }\
...
- (BOOL) tableView:(NSTV*)tV   acceptDrop:(IDDRAG)info		   row:(NSI)row 	      dropOperation:(NSTVDO)op	{\
	...
	[[self valueForKey:mutableObjectArrayKey] moveObjectAtIndex:dragRow toIndex:row];\
	return YES;}\
