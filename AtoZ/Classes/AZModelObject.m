...
+ (void) initialize {
	...
	for (Class cls = self; cls != [AZModelObject class]; cls = [cls superclass]) {
...
- (void) writeToDescription:(NSMutableString *)description withIndent:(NSUInteger)indent {
	...
	for (NSString *name in [self allKeys]) {
		...
		if ([object isKindOfClass:[AZModelObject class]]) {
			...
			for (id child in object) {
				...
				if ([child isKindOfClass:[AZModelObject class]])
