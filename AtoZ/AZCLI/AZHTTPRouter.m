...
- (void) start
{
	...
	self.queriesController = [NSArrayController.alloc init];
	...
	[@[	@[ @"/hello",			       @"/hello" ],	@[ @"/hello/:name",			@"/hello/somename"],
	 ...
	 eachWithIndex:^(id obj, NSI idx) {	[_queriesController insertObject:$SHORT(obj[0], obj[1]) atArrangedObjectIndex:idx];
		 ...
		 [self get:obj[0] withBlock:^(REQ *req, RES *res) {	[res respondWithString
	...
	//	[self get:@"/info" withBlock:^(REQ *req, RES *res) { AZLOG(req);	AZLOG(res);	}];
