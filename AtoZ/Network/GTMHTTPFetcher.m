...
#define GTMHTTPFETCHER_DEFINE_GLOBALS 0
...
- (void) ailToBeginFetchWithError:(NSError *)error;

#if GTM_BACKGROUND_FETCHING
- (void) ndBackgroundTask;
- (void) ackgroundFetchExpired;
#endif

- (BOOL)authorizeRequest;
- (void) uthorizer:(id <GTMFetcherAuthorizationProtocol>)auth
		   request:(NSMutableURLRequest *)request
 finishedWithError:(NSError *)error;

- (NSS*) createTempDownloadFilePathForPath:(NSS*) targetPath;
- (NSFileManager *)fileManager;
- (void) topFetchReleasingCallbacks:(BOOL)shouldReleaseCallbacks;
- (BOOL)shouldReleaseCallbacksUponCompletion;

- (void) ddCookiesToRequest:(NSMutableURLRequest *)request;
- (void) andleCookiesForResponse:(NSURLResponse *)response;

- (void) ogNowWithError:(NSError *)error;

- (void) nvokeFetchCallbacksWithData:(NSData *)data
							   error:(NSError *)error;
- (void) nvokeFetchCallback:(SEL)sel
					 target:(id)target
					   data:(NSData *)data
					  error:(NSError *)error;
- (void) eleaseCallbacks;

- (void) onnection:(NSURLConnection *)connection didFailWithError:(NSError *)error;

- (BOOL)shouldRetryNowForStatus:(NSInteger)status error:(NSError *)error;
- (void) estroyRetryTimer;
- (void) eginRetryTimer;
- (void) rimeRetryTimerWithNewTimeInterval:(NSTimeInterval)secs;
- (void) endStopNotificationIfNeeded;
- (void) setryFetch;
- (void) setryTimerFired:(NSTimer *)timer;
@end

@interface GTMHTTPFetcher (GTMHTTPFetcherLoggingInternal)
- (void) setupStreamLogging;
- (void) ogFetchWithError:(NSError *)error;
@end

@implementation GTMHTTPFetcher

+ (GTMHTTPFetcher *)fetcherWithRequest:(NSURLRequest *)request {
  return [[[[self class] alloc] initWithRequest:(id)request] autorelease];
}
...
+ (void)initialize {
  ...
  if (!gGTMFetcherStaticCookieStorage) {
	...
	if (cookieStorageClass) {
	  gGTMFetcherStaticCookieStorage = cookieStorageClass.new;
	}
  }
}
...
- (void) aitForCompletionWithTimeout:(NSTimeInterval)timeoutInSeconds {
...
- (void) onnection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
...
- (void) onnection:(NSURLConnection *)connection
...
- (void) nvokeSentDataCallback:(SEL)sel
						target:(id)target
...
- (void) onnection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
#if DEBUG
...
- (void) onnectionDidFinishLoading:(NSURLConnection *)connection {
...
- (void) ddPropertiesFromDictionary:(NSDictionary *)dict {
