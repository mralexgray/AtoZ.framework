...
- (void) reset { [self initWithFrame:self.bounds];  }///[@[@"color", @"dictionary", ]]

- (AHLayout*) parentLayout 		{ return (AHLayout*) self.superview;	}

- (void) setTag:(NSI) tag 	 		{	[super setTag:tag]; }		/*+ (NSSet*) keyPathsForValuesAffectingValueForKey:(NSS*) key {
																					 
																					 NSSet *kps = [[super keyPathsForValuesAffectingValueForKey:key] setByAddingObjectsFromArray:
...
- (NSAS*) attrString	 				{ NSS*s = _name ?: _url.host.stringValue ?: NSS.randomDicksonism; return [s attributedWithFont:AtoZ.controlFont andColor:WHITE];	}

- (NSC*) color 						{  return _color ?: dictionary[@"color"] ?: self.favicon.quantized;  }

- (NSURL*) url 						{ 	return _url ?: [dictionary[@"url"] urlified]; }

- (NSImage *)favicon {  return _favicon ?: dictionary[@"favicon"] ?: AZFavIconManager.sharedInstance.placehoder;
//	
...
- (void) toggleExpanded 		{	CGSZ tSize; CGF w, h; BOOL isVRT = !self.parentLayout.typeOfLayout == AHLayoutHorizontal;

	if (!isVRT) { 	w 		= expanded ? originalSize.width  :  self.parentLayout.width;
					  	tSize = 											(CGS)   { w, self.height }; }
	else 			{	h 		= expanded ? originalSize.height : self.parentLayout.height;
