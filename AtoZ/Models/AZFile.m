...
- (NSC *)color   {
    return _color = _color ? _color : [self.colors normal:0][@"color"];
}
...
+ (instancetype)instanceWithPath:(NSString *)path {
    return [[[self class] alloc]initWithObject:path];
}
...
+ (instancetype)dummy {
    return [[AZFile alloc]initWithObject:[[NSBundle bundleForClass:[AtoZ class]]pathForImageResource:@"missing.png"]];
}
...
//@prop_RO NSA *files;
//@prop_RO NSA *folders;
@prop_RO NSA *appFolder;
//@prop_RO NSA *categories;
@end
